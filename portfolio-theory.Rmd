...
Since we will spend so much time on data wrangling, we won't look at different coding paradigms in this chapter. It will be all tidy.
...
### Another Path for Wrangling {-}

Let's look at how we could have changed our returns object `portfolio_returns_tq_rebalanced_monthly` to match the Fama French factors. 

We want to recast the `date` column to be tied to the first of the month. If it were an `xts` object, we could use `to.monthly(indexAt = "firstof", OHLC = FALSE)`. So, let's convert it to `xts` with 
`tk_xts(date_var = date)` from the `timetk` package first, and then run our operation.  
...
The dates are in the same format now. This makes our life a lot easier - but there was a cost. We changed our base data object. Was it worth it? Well, we can perform a join to put these into one object now. But first we have to convert the ` portfolio_returns_tq_rebalanced_monthly_first_day` back to a tibble with `tk_tbl(preserve_index = TRUE, rename_index = "date")`.
...
Why stop now - let's see if we can create a Shiny app for fitting FF models that uses from FF's website. 

### Shiny Fama French {-}

Strangely enough, we have more freedom with our Shiny app than we do with the calculations above precisly because we have to give the end user freedom to build a custom portfolio from scratch. This freedom for the end users means we can recast our returns object to the first of the month. We're already starting from scratch afterall. 

Here's the link and snapshot at the app:

http://www.reproduciblefinance.com/shiny/fama-french-three-factor/

```{r, echo=FALSE, fig.cap = "Fama French Shiny", out.width='100%'}
knitr::include_graphics("snapshots/ff-shiny.png")
```


Before we even get to the inpute sidebar, have a look at the `setup` code chunk. We load packages, and we download the FF csv.
...
Our input sidebar will be the same as usual but when we calculate returns, we set

`to.monthly(indexAt = "firstof", OHLC = FALSE)`.  We have the freedom to change our data to the first of the month periodicity from the very beginning. 
...
Now we load and wrangle the FF factors. We are going to filter based on the user inputed date which is the first date in the reactive `portfolio_returns_tq_rebalanced_monthly_first_day`.
...
Now merge `portfolio_returns_tq_rebalanced_monthly_first_day` and `Global_3_Factors` into one object wiht a `left_join()`.
...
Now fit our model on that object with `do(model = lm(Returns_excess ~ MKT + SMB + HML, data = .))`. 
...
And display the results with `renderTable()`. 
...
```{r, echo=FALSE, fig.cap = "FF multi linear regression results", out.width='100%'}
knitr::include_graphics("snapshots/ff-table.png")
```

Porting our work over to Shiny was much less cumbersome than our original grinding, or maybe it seemed so because we spent so much effort on the dates from the outset. 

There are now two data sources for this Shiny app, and that means two possible points of failure that are out of our control. FF could decide to take down this data, or change its format, and we wouldn't know about it. And, yahoo! Finance could do the same. In practice, in industry, that would be less likely. We hope that if our database administrators change how data is stored or accessed, they will inform teams that depend upon them. Hope isn't a great strategy, though, so be sure make friends with the database administrators, or, better yet, fight whatever fight you need to (usually hiring budget - but it's worth it!) in order to hire on a DBA as part of your team. 

Before we wrap this up, have another look at the FF website and you will see loads and loads of data sets, different factors that can be tested as explanatory variables. How could we wire up our app so that the user could choose his own factors? What would be the importa and wrangling challenges? 

Maybe we'll tackle that in the future but for now it's a wrap on our Fama French work. 

### Concluding Portfolio Theory {-}

We reduced a vast and fascinating field to four areas, but hopefully those four lay the groundwork for more complex work by you and your team.